...
CLabyrinthe::CLabyrinthe(const unsigned & y, const unsigned & x) noexcept : m_Y(y), m_X(x), solved(false)
{
...
void CLabyrinthe::AfficherPlusChemin(ofstream &bobstream)
{
    ...
    for (unsigned i(0); i < m_Labyrinthe.size(); ++i)
    {
        for (unsigned j(0); j < m_Labyrinthe[i].size(); ++j)
        {
            if(berthier.GetLabyrinthe()[i][j].GetOuest() || berthier.GetLabyrinthe()[i][j].GetEst() || berthier.GetLabyrinthe()[i][j].GetNord() || berthier.GetLabyrinthe()[i][j].GetSud())
            {
                m_Labyrinthe[i][j].setId(idpath);
            }
        }
    }
    clock_t end = clock();
    ...
    for (unsigned i(0); i < m_Labyrinthe.size(); ++i)
    {
        for (unsigned j(0); j < m_Labyrinthe[i].size(); ++j)
        {
            ...
            if(m_Labyrinthe[i][j].GetId()==idpath && m_Labyrinthe[i-1][j].GetId()==idpath && m_Labyrinthe[i][j].GetOuest())
                buffer += chemin;
            else if (m_Labyrinthe[i][j].GetOuest())
                buffer += vide;
            else
        ...
        for (unsigned j(0); j < m_Labyrinthe[i].size(); ++j)
        {
            if(m_Labyrinthe[i][j].GetNord() && j==0)
                buffer += chemin;
            else if(m_Labyrinthe[i][j].GetId()==idpath && m_Labyrinthe[i][j-1].GetId()==idpath && m_Labyrinthe[i][j].GetNord())
                buffer += chemin;
            else if (m_Labyrinthe[i][j].GetNord())
                buffer += vide;
            else
                buffer += mur;
            if (m_Labyrinthe[i][j].GetId()==idpath)
                buffer += chemin;
            else
                buffer += vide ;
            if (j == m_Labyrinthe[i].size() - 1)
            {
                if(m_Labyrinthe[i][j].GetSud())
                    buffer += chemin;
                else if(m_Labyrinthe[i][j].GetId()==idpath && m_Labyrinthe[i][j+1].GetId()==idpath && m_Labyrinthe[i][j].GetSud())
                    buffer += chemin;
                else
                    buffer += mur;
            }
